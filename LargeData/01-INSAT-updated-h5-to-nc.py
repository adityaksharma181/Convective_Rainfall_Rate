...
# 1. Processes 100 files at a time in batches
def process_all_files_parallel(input_dir, output_dir):
    ...
    for i in range(0, len(files), batch_size):
        ...
        # Step 1: Parallel reading/preparation of file data for this batch
        with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
            file_data_list = list(executor.map(prepare_file_data, batch_files))

        # Step 2: GPU-based interpolation and saving (sequential for this batch)
        for data in file_data_list:
            process_on_gpu(data, output_dir)

# Run the full pipeline
process_all_files_parallel("/kaggle/input/week-1n2", "/kaggle/working")
