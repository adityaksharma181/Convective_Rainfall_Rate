...
# Flatten the target arrays (y) for model compatibility
y_train_flat = y_train.reshape((y_train.shape[0], -1))  # Flatten for train
y_val_flat   = y_val.reshape((y_val.shape[0], -1))  # Flatten for validation
y_test_flat  = y_test.reshape((y_test.shape[0], -1))  # Flatten for test

# Define a custom weighted Mean Squared Error (MSE) loss function
# This gives higher penalty for errors on high-rainfall areas
def weighted_mse(y_true, y_pred):
    weight = (
        1.0  # Base weight
        + 5.0 * tf.cast(y_true > 20.0, tf.float32)  # Extra weight if rainfall > 20mm
        + 10.0 * tf.cast(y_true > 40.0, tf.float32)  # Even more weight if > 40mm
    )
